    ...
    while (1)
    {
        ...
        while (mod_i < 2)
        {
            ...
            while (mod_j < 2)
            {
                ...
                if (map[i][j])
                {
                    if ((map[i][j] == '0' || map[i][j] == 'P') && !map_costs[i][j].passed)
                    {
                        if ((map_costs[i][j].gcost == -1 && map_costs[i][j].hcost == -1) || (get_gcost(j, i, y, x) <= map_costs[i][j].gcost && get_hcost(map, j, i) < map_costs[i][j].hcost))
                        {
